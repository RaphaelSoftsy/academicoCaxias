...
$(document).ready(function() {
	...
	$.ajax({
		...
		$.each(data, function(index, item) {
			$('#idDestinacaoLixoEdit').append($('<option>', {
				value: item.idDestinacaoLixo,
				text: item.destinacaoLixo,
				name: item.destinacaoLixo
			}));
		});
		$.each(data, function(index, item) {
			$('#idDestinacaoLixo').append($('<option>', {
				value: item.idDestinacaoLixo,
				text: item.destinacaoLixo,
				name: item.destinacaoLixo
			}));
	...
	$('.searchButton').click(function() {
		...
		if (columnToSearch === 'destinacaoLixo') {
			filteredData = dadosOriginais.filter(function(item) {
				return item.destinacaoLixo.destinacaoLixo.toLowerCase().includes(searchInput);
			});
	...
	function sortData(column, order) {
		...
		dadosOrdenados.sort(function(a, b) {
			if (column === 'destinacaoLixo') {
				var valueA = a.destinacaoLixo.destinacaoLixo.toLowerCase();
				var valueB = b.destinacaoLixo.destinacaoLixo.toLowerCase();
				if (order === 'asc') {
...
function getDados() {
	...
	$.ajax({
		...
		url: url_base + "/escolaDestinacaoLixo",
		type: "GET",
		async: false,
	})
		.done(function(data) {
...
function listarDados(dados) {
	var html = dados.map(function(item) {
		...
		return (
			...
			"</td>" +
			"<td>" +
			item.destinacaoLixo.destinacaoLixo +
			"</td>" +
			...
			item.escolaId +
			'" data-id="' +
			item.idEscolaDestinacaoLixo +
			'" data-idSelect2="' +
			item.destinacaoLixo.idDestinacaoLixo +
...
$('#exportar-excel').click(function() {
	...
	XLSX.writeFile(livro, "destinacaoDeLixo.xlsx");
});
...
function showModal(ref) {
	...
	$("#idDestinacaoLixoEdit").val(idSelect2).attr('selected', true);
}
...
function editar() {
	var objeto = {
		idEscolaDestinacaoLixo: Number(id),
		escolaId: Number(idEscola),
		destinacaoLixoId: Number($('#idDestinacaoLixoEdit').val())
	}
...
function cadastrar() {
	...
	var objeto = {
		escolaId: Number(idEscola),
		destinacaoLixoId: Number($('#idDestinacaoLixo').val())
	}
