...
$(document).ready(function() {
	...
	$.ajax({
		...
		.done(function(data) {
			$.each(data, function(index, item) {
				$('#turmaDisciplinaId').append($('<option>', {
			...
			$.each(data, function(index, item) {
				$('#turmaDisciplinaIdEdit').append($('<option>', {
					value: item.idTurmaDisciplina,
					text: item.disciplina.nome,
					name: item.disciplina.nome
				}));
	...
	$('.searchButton').click(function() {
		...
		if (columnToSearch === 'turmaDisciplina') {
			filteredData = dadosOriginais.filter(function(item) {
				return item.turmaDisciplina.disciplina.nome.toLowerCase().includes(searchInput);
			});
...
function showModal(ref) {
	...
	$.ajax({
		...
		.done(function(ref) {
			$("#turmaDisciplinaIdEdit").val(ref.turmaDisciplina.disciplina.idDisciplina).attr('selected', true);
			$("#professorIdEdit").val(ref.professorId).attr('selected', true);
			$("#tipoProfessorEdit").val(ref.tipoProfessor).attr('selected', true);
			$("#tipoVagaEdit").val(ref.tipoVaga).attr('selected', true);
		})
}
...
function editar() {
	...
	var objeto = {
		idTurmaProfessor: id,
		turmaId: $("#turmaDisciplinaIdEdit").val(),
...
function cadastrar() {
	var objeto = {
		...
		turmaDisciplinaIdEdit: $("#turmaDisciplinaIdEdit").val(),
